...
tags:
  - Category/District
MyContainer:
  - "[[2-World/Hubs/Test City.md|Test City]]"
MyCategory:
...
// 1) Rename if title starts with "NewDistrict"
let title;
if (tp.file.title.startsWith("NewDistrict")) {
  title = await tp.system.prompt("Enter Hub District");
  if (!title) {
...
const regionFiles = tp.app.vault.getMarkdownFiles()
  .filter(f => f.path.startsWith("2-World/Districts/"));

const placeholderLabel = "🌀 No District Selected";
